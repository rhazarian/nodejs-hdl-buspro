...
const {isBuffer} = require("mocha/lib/utils");
...
Bus.prototype.send = function(target, code, data, callback) {
  this.sendAs(this, target, code, data, callback);
}

/**
 * Send command (with custom sender) to device via HDL SmartBus gateway
 *
 * @param  {String|Device}  sender   Device address or object
 * @param  {String|Device}  target   Device address or object
 * @param  {Number}         code     Command code
 * @param  {Object|Buffer}  [data]     Additional data
 * @param  {Function}       callback
 */
Bus.prototype.sendAs = function(sender, target, code, data, callback) {
